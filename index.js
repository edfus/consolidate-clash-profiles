...
const [profileTemplate, fetchedProxies] = await Promise.all([
  fsp.readFile("./template.yml", "utf-8"),
  Promise.allSettled(
    profiles.map(
      async profile => {
...
for (const doh of dohs) {
  if(!combinedProfile.dns.nameserver.includes(doh)) {
    try {
      ...
      combinedProfile.dns.nameserver.push(doh);
    } catch (err) {
      console.error(err);
    }
  }
...
await fsp.writeFile("output.yml", dump(combinedProfile), "utf-8");
